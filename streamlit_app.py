...
def load_data():
    try:
        ...
        # Process fixed_time
        def clean_fixed_time(x, mean_value=30):
            if pd.isna(x):
                return np.nan
            x_str = str(x).lower()
            numbers = re.findall(r'\d+', x_str)
            if numbers:
                return int(numbers[0])  # Возвращаем первое число как float
...
fixed_time = st.sidebar.slider("Длительность перемирия (дни)", 0.00, float(df['fixed_time'].max()), float(df['fixed_time'].median()))
